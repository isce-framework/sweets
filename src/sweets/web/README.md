...
With "Show advanced" on, the nested `DolphinOptions` and `TropoOptions`
pydantic models are folded in — phase-linking window, strides, unwrap
method, snaphu / spurt knobs, tropo step toggles, etc.:
...
With pixi, the same loop is one-liner per terminal:

```bash
pixi run -e web sweets-server          # backend
pixi run -e web web-install            # one-time: npm install
pixi run -e web web-dev                # frontend
```
...
The README images are regenerated by `scripts/web_screenshots.py`, which:
1. Rebuilds the frontend bundle (`npm run build`).
2. Boots a temporary uvicorn server on a free port.
3. Drives Playwright through every demo flow — search, track-narrow,
   granule list expand, advanced config dolphin / tropo sections, jobs.
4. Writes PNGs into `docs/web/screenshots/`.
5. Kills the server.

```bash
# One-time setup (inside the web pixi env):
playwright install chromium

# Then, whenever the UI changes:
pixi run -e web web-screenshots
```

The search step hits live ASF / CMR APIs to give a realistic results
overlay; no credentials are required for the public granule queries.
CI does not run this — it needs a GUI browser binary and live network.
Refresh manually after any layout change or before publishing a new
demo to the team.
