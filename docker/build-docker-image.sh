...
CMD_BASE="docker build --network=host --tag isce-framework/sweets:$TAG --file docker/Dockerfile"
...
echo "docker run --rm -it isce-framework/sweets:$TAG sweets --help"
#
echo "To run the workflow on a configuration in the current directory...."
echo "docker run --user \$(id -u):\$(id -g) -v \$PWD:/work --rm -it isce-framework/sweets:$TAG sweets run sweets_config.yaml"
# where...
...
#     isce-framework/sweets:latest  # The name of the image to run.
#     sweets run sweets_config.yaml # The `sweets` command that is run in the container
